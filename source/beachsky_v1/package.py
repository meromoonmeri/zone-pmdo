...
def imgs(m):
    ...
    for f in man['modes'][m]['calques']:
        k=f.split('_',3)[3].split('_')[0]; ph=f.split('PHASE')[1][:2] if 'PHASE' in f else ''
        cls='L'+((' ph ph'+k) if ph else '')+(' cl' if k=='01' else ''); out.append(f"<img class='{cls}' data-n='{k}' data-ph='{ph}' src='{b64(HERE/'couches'/f)}'>")
    return ''.join(out)
labels={'00':'Ciel natif','01':'Nuages (bande défilante)','02':'(réservé)','03':f"Mer : vagues canon Beach ({man['phases_eau']} frames dessinées)",'04':'Sable (plaque validée)','04b':f"Rivage sur le sable ({man['phases_rivage']} frames : monte / se retire)",'05':'Sentier roche / rebord ocre','06':'Ombres calculées','07':'Falaises nord','08':'Profondeur grotte','09':'Pointes rocheuses','10':'Herbes','11':'Palmiers'}
...
<p><small>Composition validée par l'utilisateur, calques régénérés sur magenta ; eau = canon Beach mesuré : base fixe + {man['phases_eau']} frames dessinées de vagues + {man['phases_rivage']} frames de rivage (frames explicites, 10 ticks) ; nuages = bande 1440 px à −4 px/s ; nuit = filtre Abyss exact ; crépuscule = interpolation 45 % + voile chaud. <b>Aperçu de fichiers, pas de test moteur.</b></small></p>
...
<script>
const N3={man['phases_eau']},N4={man['phases_rivage']};let t=0,cx=0;
...
function tick(){{const a=document.getElementById('anim').checked;if(a)t++;document.querySelectorAll('img.ph03').forEach(i=>i.style.visibility=(i.dataset.off||(+i.dataset.ph!==t%N3))?'hidden':'visible');document.querySelectorAll('img.ph04b').forEach(i=>i.style.visibility=(i.dataset.off||(+i.dataset.ph!==t%N4))?'hidden':'visible');
document.querySelectorAll('img.L:not(.ph)').forEach(i=>i.style.visibility=i.dataset.off?'hidden':'visible');}}
setInterval(tick,167);
